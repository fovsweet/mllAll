	      ...
	      $.ajax({
	        ...
	        success: function (data) {
	          ...
	          if(data.headImgUrl != '' && data.headImgUrl != null){
	          	me.headImg = data.headImgUrl;
	          }
	          me.headImgUrl = data.personalCenterUrl;
	        }
	      });
	    },
	    methods: {
	      down_a: function () {
	        ...
	        me.$options.loadListData(function (data) {
	            me.content = me.content.concat(data.content)
	            // 通过设置的key 方法下拉对象方法
	            // 如果没有更多数据。你可以 调用 me.ascroll.noData()
	            if(data.totalPages <= 1){
		          	$('.dropload-down .dropload-load').html('')
		         }else {
		         	if(data.content.length < 1){
		         		//me.ascroll.noData();
	            	$('.dropload-down .dropload-load').html('没有更多数据了')
	      ...
	      up_a: function () {
	        ...
	        me.$options.loadListData(function (data) {
	              ...
	              $('.dropload-down .dropload-load').html('数据加载中...')
...
<template>
		...
		<div>
		   ...
		   <a :href="data.collectInitUrl">
		 	<div class="list-item" >
		 		...
		 		<div class="list-info">
		 			<span class='list-info-title'>{{data.title}}</span>
		 			<div class="action-wraper">
			 			<div class="timer">{{data.updateTimeStr}}</div>
			 			<div class="action" @click.stop.prevent="setting(data.sysUuid)"></div>
		 			</div>
		 		</div>		 		
		 	</div>
		 	</a>
	...
	<a class="kefu" :href="headImgUrl">
		<img :src="headImg">
	</a>
	<div :class="{nostore:content.length < 1}" ></div>
</template>
...
<style lang="less">
	...
	.nostore{
		...
		top: 0;
		left: 0;
		background:#f4f4f4 url(../assents/images/nostore.png) no-repeat;
